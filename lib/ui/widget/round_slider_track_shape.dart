...
class RoundSliderTrackShape extends SliderTrackShape {
  ...
  Rect getPreferredRect({
    ...
    final double trackWidth = (parentBox?.size.width ?? 0) - overlayWidth;
    return Rect.fromLTWH(trackLeft ?? 0, trackTop, trackWidth, trackHeight);
  }
  ...
  void paint(
    ...
    final Rect leftTrackSegment = Rect.fromLTRB(trackRect.left, trackRect.top,
        (thumbCenter?.dx ?? 0) - horizontalAdjustment, trackRect.bottom);

    // Left Arc
    context.canvas.drawArc(
        Rect.fromCircle(
            center: Offset(trackRect.left, trackRect.top + 11.0), radius: 5.0),
        -pi * 3 / 2, // -270 degrees
        pi, // 180 degrees
        false,
    ...
    context.canvas.drawArc(
        ...
        pi, // 180 degrees
        false,
        trackRect.right - (thumbCenter?.dx ?? 0) == 0.0
            ? leftTrackPaint
            : rightTrackPaint);

    context.canvas.drawRect(leftTrackSegment, leftTrackPaint);
    final Rect rightTrackSegment = Rect.fromLTRB(
        (thumbCenter?.dx ?? 0) + horizontalAdjustment,
        trackRect.top,
        trackRect.right,
        trackRect.bottom);
